...
Usage:
    python inspect_h5_hu.py /Data2/huxiaowan/ysq/RefineFlow-MAR/test_data/oral_ct_Cu/test_720geo/patient_0000/000/9.h5 --save-png
"""
...
# ============ Physical constants ============
MIU_WATER = 0.192  # Linear attenuation coefficient of water at 70 keV (cm^-1)

# ============ Dental CT HU range ============
HU_MIN = -1000.0   # Lower bound: air and soft tissue
HU_MAX = 3000.0    # Upper bound: metal and high-density bone


def attenuation_to_hu(mu):
    """
    Convert attenuation coefficient mu (cm^-1) to HU.
    Formula: HU = (mu - mu_water) / mu_water * 1000
    
    Args:
        mu: Attenuation coefficient in cm^-1, usually in [0, 0.4].
    
    Returns:
        HU values, usually in [-1000, 3000+].
    """
    return (mu - MIU_WATER) / MIU_WATER * 1000.0


def hu_to_attenuation(hu):
    """
    Convert HU to attenuation coefficient mu (cm^-1).
    Formula: mu = HU / 1000 * mu_water + mu_water
    ...
    """
    return hu / 1000.0 * MIU_WATER + MIU_WATER
...
def normalize_for_display(data, name, bit_depth=16, use_hu_window=True, hu_min=HU_MIN, hu_max=HU_MAX):
    """Normalize data to the requested bit depth for display and saving.
    
    For medical CT data in DuDoDp format:
      - Image domain (image, ma_CT, LI_CT):
          * use_hu_window=True: attenuation -> HU -> clip -> normalize.
          * use_hu_window=False: use the original min-max normalization.
      - Sinogram domain: use min-max normalization independently of the HU window.
      - Mask domain (metal_trace): map {0, 1} directly to {0, max_output}.
    
    Args:
        data: Input float32/float64 data.
        name: Dataset name.
        bit_depth: Output bit depth, either 8 or 16.
        use_hu_window: Apply HU-window normalization to CT images when true.
        hu_min: Lower CT-image HU bound, used only with an HU window.
        hu_max: Upper CT-image HU bound, used only with an HU window.
    ...
    # Keep image-domain and sinogram-domain normalization separate.
    name_lower = name.lower()
    is_sinogram = ('sinogram' in name_lower or 'sino' in name_lower)
    is_ct_image = ('image' in name_lower or 'ct' in name_lower or name_lower.endswith('_ct')) and not is_sinogram
    
    # Image domain: select HU-window or min-max normalization for CT images.
    if is_ct_image:
        if use_hu_window:
            # HU-window normalization is recommended for dental CT.
            # Convert attenuation coefficients to HU.
            data_hu = attenuation_to_hu(data)
            
            # Clip to the requested HU range and normalize to [0, max_output].
            normalized = np.clip(data_hu, hu_min, hu_max)
            normalized = (normalized - hu_min) / (hu_max - hu_min) * max_output
            
            print(f"  📊 [Image domain] HU normalization: [{hu_min:.0f}, {hu_max:.0f}] HU -> [0, {max_output}]")
            print(f"     Original μ: [{min_val:.4f}, {max_val:.4f}] cm⁻¹")
            print(f"     Original HU: [{attenuation_to_hu(min_val):.1f}, {attenuation_to_hu(max_val):.1f}]")
        else:
            # Original min-max normalization method
            normalized = (data - min_val) / (max_val - min_val) * max_output
            print(f"  📊 [Image domain] Min-Max normalization: [{min_val:.4f}, {max_val:.4f}] -> [0, {max_output}]")
    
    # Mask domain: map binary data such as metal_trace directly.
    elif max_val <= 1.01 and len(np.unique(data)) <= 3:
        normalized = data * max_output
        print(f"  📊 [Mask] Binary mapping: {{0, 1}} -> {{0, {max_output}}}")
    
    # Sinogram domain: use min-max normalization independently of the HU window.
    else:
        normalized = (data - min_val) / (max_val - min_val) * max_output
        domain_type = "[Sinogram domain]" if is_sinogram else "[Other]"
        print(f"  📊 {domain_type} Min-Max normalization: [{min_val:.4f}, {max_val:.4f}] -> [0, {max_output}]")
...
def save_image_as_png(data, output_path, name, bit_depth=16, colormap='gray',
                     use_hu_window=True, hu_min=HU_MIN, hu_max=HU_MAX):
    """Save data as a PNG image.
    
    Select the normalization strategy from the dataset name:
      - Image domain: HU clipping when enabled, otherwise min-max normalization.
      - Sinogram domain: min-max normalization independently of the HU window.
      - Mask domain: binary mapping from {0, 1} to {0, max_output}.
    ...
    if len(data.shape) == 2:
        ...
        img = normalize_for_display(data, name, bit_depth, use_hu_window=use_hu_window, hu_min=hu_min, hu_max=hu_max)
        ...
        if data.shape[0] <= 3:
            ...
            img = normalize_for_display(data[0], name, bit_depth, use_hu_window=use_hu_window, hu_min=hu_min, hu_max=hu_max)
            cv2.imwrite(str(output_path), img)
            ...
            img = normalize_for_display(data[mid_slice], name, bit_depth, use_hu_window=use_hu_window, hu_min=hu_min, hu_max=hu_max)
            cv2.imwrite(str(output_path), img)
        ...
        img = normalize_for_display(data[0, 0], name, bit_depth, use_hu_window=use_hu_window, hu_min=hu_min, hu_max=hu_max)
        cv2.imwrite(str(output_path), img)
        ...
        if save_png and output_dir:
            ...
            for name, data in datasets_dict.items():
                ...
                save_image_as_png(data, png_path, name, bit_depth=bit_depth,
                                use_hu_window=use_hu_window, hu_min=hu_min, hu_max=hu_max)
...
Examples:
  ...
  python inspect_h5_data.py data/*.h5 --save-png --output-dir ./output
    ...
    parser.add_argument('--use-hu-window', action='store_true', default=True,
                        help='Use HU window normalization for CT images (default: True)')
    parser.add_argument('--no-hu-window', dest='use_hu_window', action='store_false',
                        help='Use min-max normalization for CT images (original method)')
    parser.add_argument('--hu-min', type=float, default=-1000.0,
                        help='HU minimum value for CT images (default: -1000, only when --use-hu-window)')
    parser.add_argument('--hu-max', type=float, default=3000.0,
                        help='HU maximum value for CT images (default: 3000, only when --use-hu-window)')
    
    args = parser.parse_args()
    
    # Display HU settings only when PNG output is enabled.
    if args.save_png:
        print(f"\n🔧 CT image normalization settings:")
        if args.use_hu_window:
            print(f"   [Image domain] Fixed HU window: [{args.hu_min:.0f}, {args.hu_max:.0f}] HU")
            print(f"   (Adjust with --hu-min/--hu-max, or use --no-hu-window for min-max normalization)")
        else:
            print(f"   [Image domain] Min-max normalization (original method)")
            print(f"   (Use --use-hu-window to enable HU-window normalization)")
        print(f"   [Sinogram domain] Automatic min-max normalization, independent of the HU window\n")
    ...
    for h5_file in h5_files:
        inspect_h5_file(
            ...
            use_hu_window=args.use_hu_window,
            hu_min=args.hu_min,
            hu_max=args.hu_max
        )
